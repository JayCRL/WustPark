...
var SITE_SECTIONS = ['clubs','activities','market','teams','info','cats','messages','profile','admin','friends'];
function getPageContext() {
  var parts = location.pathname.replace(/\/+$/, '').split('/').filter(Boolean);
  var file = parts[parts.length - 1] || 'index.html';
  var parent = parts.length > 1 ? parts[parts.length - 2] : '';
  var section = 'home';
  if (SITE_SECTIONS.includes(file) && !file.includes('.')) {
    section = file;
    file = 'index.html';
  } else if (SITE_SECTIONS.includes(parent)) {
    section = parent;
  } else if (file !== 'index.html') {
    section = file.replace(/\.html$/, '');
  }
  if (file === 'index.html' && !SITE_SECTIONS.includes(section)) section = 'home';
  return {
    file: file,
    section: section,
    prefix: SITE_SECTIONS.includes(section) ? '../' : ''
  };
}
function sectionIndexHref(section) {
  var ctx = getPageContext();
  return ctx.section === section ? 'index.html' : ctx.prefix + section + '/index.html';
}
function sectionDetailHref(section, id, hash) {
  var ctx = getPageContext();
  var detailFile = 'detail.v20260701c.html';
  var base = ctx.section === section ? detailFile : ctx.prefix + section + '/' + detailFile;
  return base + '?id=' + encodeURIComponent(id) + (hash || '');
}
function clubsIndexHref() { return sectionIndexHref('clubs'); }
function clubDetailHref(id, hash) { return sectionDetailHref('clubs', id, hash); }
function activitiesIndexHref() { return sectionIndexHref('activities'); }
function activityDetailHref(id) { return sectionDetailHref('activities', id); }
// ============================================================
...
// 渲染函数
// ============================================================
function getClubTypeMeta(type) {
  var map = {
    club: { name: '社团', icon: '🎪' },
    department: { name: '部门', icon: '🏛️' },
    interest_group: { name: '兴趣组', icon: '💡' }
  };
...
function renderClubCard(club) {
  ...
  return `
    <a href="${clubDetailHref(club.id)}" class="club-card-link">
...
function renderActivityCard(activity) {
  ...
  return `
    <a href="${activityDetailHref(activity.id)}" class="activity-card-link">
  ...
  function setup(c) {
    ...
    c.addEventListener('click', function(e) {
      ...
      if (c === typeC) {
        ...
        document.querySelectorAll('#filterTags .tag-chip[data-group]').forEach(function(x){ if (x.dataset.group !== 'all') x.classList.remove('active'); });
      }
      renderAllClubs();
    });
  }
...
function renderActivityDetail() {
  ...
  fetchActivityDetailFromAPI(activityId).then(function(result) {
    if (!result || !result.activity) {
      container.innerHTML = '<div class="empty-state" style="padding:6rem 2rem;"><span class="empty-icon">🔍</span><h3>活动不存在</h3><p>请检查活动 ID 是否正确</p><a href="' + activitiesIndexHref() + '" class="btn btn-primary" style="margin-top:1rem;">返回活动列表</a></div>';
      return;
    }
    ...
    container.innerHTML =
      ...
      tipsHtml +
      clubHtml +
      (isUpcoming ? '<section class="cta-section"><div class="container"><div class="cta-content"><h2>🎉 对活动感兴趣？</h2><p>了解更多活动详情，欢迎联系主办社团</p><div style="display:flex;gap:var(--space-md);justify-content:center;flex-wrap:wrap;">' + (club ? '<a href="' + clubDetailHref(club.id, '#join-club') + '" class="btn btn-white btn-lg">联系 ' + club.name + '</a>' : '') + '<a href="' + activitiesIndexHref() + '" class="btn btn-outline btn-lg" style="border-color:white;color:white;">浏览更多活动</a></div></div></div></section>' : '');
    observeAnimatedElements();
...
function genCta(club) {
  var link = club ? '<a href="' + clubDetailHref(club.id, '#join-club') + '" class="btn btn-white btn-lg">联系 ' + club.name + '</a>' : '';
  return '<section class="cta-section"><div class="container"><div class="cta-content"><h2>🎉 对活动感兴趣？</h2><p>了解更多活动详情，欢迎联系主办社团</p><div style="display:flex;gap:var(--space-md);justify-content:center;flex-wrap:wrap;">' + link + '<a href="' + activitiesIndexHref() + '" class="btn btn-outline btn-lg" style="border-color:white;color:white;">浏览更多活动</a></div></div></div></section>';
}
...
function getClubRoleName(role, type) {
  if (role === 'president') {
    if (type === 'interest_group') return '组长';
    if (type === 'department') return '负责人';
    return '社长';
  }
  if (role === 'vice_president') {
    if (type === 'interest_group') return '副组长';
    if (type === 'department') return '副负责人';
    return '副社长';
  }
  return '成员';
}
function renderClubJoinAction(club, mode) {
  var typeMeta = getClubTypeMeta(club.type);
  var primaryClass = 'btn btn-white btn-lg';
  var outlineClass = 'btn btn-outline btn-lg';
  var outlineStyle = mode === 'hero' ? 'border-color:rgba(255,255,255,0.3);color:#fff;' : 'border-color:white;color:white;';
  var membership = club.membership || null;
  if (!getToken()) {
    return '<button class="' + primaryClass + '" onclick="location.href=\'login.html\'">登录后申请加入</button>';
  }
  if (membership && membership.status === 'active') {
    var roleName = getClubRoleName(membership.role, club.type);
    if (membership.role === 'president' || membership.role === 'vice_president') {
      return '<button class="' + primaryClass + '" onclick="location.href=\'club-manage.html\'">已加入 · ' + roleName + '，去管理</button>';
    }
    return '<span class="' + primaryClass + '" style="cursor:default;">已加入 · ' + roleName + '</span>' +
      '<button class="' + outlineClass + '" style="' + outlineStyle + '" onclick="leaveClub(' + club.id + ')">退出' + typeMeta.name + '</button>';
  }
  if (membership && membership.status === 'pending') {
    return '<button class="' + primaryClass + '" style="opacity:.72;cursor:not-allowed;" disabled>申请审核中</button>';
  }
  if (membership && membership.status === 'banned') {
    return '<button class="' + primaryClass + '" style="opacity:.72;cursor:not-allowed;" disabled>暂不能申请</button>';
  }
  return '<button class="' + primaryClass + '" onclick="joinClub(' + club.id + ')">申请加入</button>';
}
  ...
  fetchClubDetailFromAPI(clubId).then(async result => {
    if (!result || !result.club) {
      container.innerHTML = `
        <div class="empty-state" style="padding:6rem 2rem;">
          ...
          <h3>社团不存在</h3>
          <p>请检查社团 ID 是否正确</p>
          <a href="${clubsIndexHref()}" class="btn btn-primary" style="margin-top:1rem;">返回社团列表</a>
        </div>
      `;
      return;
    }
    const { club, history, images } = result;
    club.membership = result.membership || null;
    var safeClubName = jsString(club.name);
    var safeContact = jsString(club.contact || '');
    var typeMeta = getClubTypeMeta(club.type);
    var clubActivities = [];
    try {
    ...
    const albumHtml = images && images.length > 0
      ...
      <section class="detail-section section-alt">
        <div class="section-header">
          <span class="section-tag">📸 ${typeMeta.name}风采</span>
    ...
    if (clubActivities.length > 0) {
      ...
      <section class="detail-section" id="clubActivitiesSection">\
        <div class="section-header">\
          <span class="section-tag">' + typeMeta.icon + ' ' + typeMeta.name + '活动</span>\
      ...
      <section class="detail-section" id="clubActivitiesSection">\
        <div class="container-narrow" style="text-align:center;padding:2rem;">\
          ...
          <p style="color:var(--text-tertiary);margin-top:0.5rem;">本' + typeMeta.name + '暂无活动</p>\
        </div>\
      </section>';
    }
    // 认领提示 - 信息不完整时显示
    ...
    container.innerHTML = claimNotice + `
      ...
      <section class="club-detail-hero" style="--hero-gradient: ${getClubGradient(club.color)};">
        ...
        <div class="container">
          <div class="cd-hero-content">
            <a href="${clubsIndexHref()}" class="cd-back-link">← 返回列表</a>
            ...
            <div class="cd-hero-actions">
              ${renderClubJoinAction(club, 'hero')}
              <button class="btn btn-outline btn-lg" style="border-color:rgba(255,255,255,0.3);color:#fff;" onclick="showContributeModal(${club.id},'${safeClubName}')">✏️ 补充信息</button>
              <button class="btn btn-outline btn-sm" style="border-color:rgba(255,255,255,0.2);color:rgba(255,255,255,0.7);" onclick="claimClub(${club.id})">🏷️ 认领${typeMeta.name}</button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section class="detail-section">
        <div class="container-narrow">
          <div class="section-header">
            <span class="section-tag">${typeMeta.icon} ${typeMeta.name}理念</span>
          ...
          <div class="join-card animate-on-scroll" style="--cta-gradient: ${getClubGradient(club.color)};">
            <div class="join-card-content">
              ...
              <div style="margin-top:1.5rem;display:flex;gap:1rem;flex-wrap:wrap;justify-content:center;">
                ${renderClubJoinAction(club, 'cta')}
                <button class="btn btn-white btn-lg" onclick="showClubContact('${safeClubName}','${safeContact}')">📞 联系方式</button>
...
function showClubContact(name, contact) {
  confirmDialog({ icon:'📞', title:(name || '组织') + ' · 联系方式', message: contact || '暂无联系方式', confirmText:'知道了', hideCancel:true });
}
...
async function joinClub(id) {
  if (!getToken()) { showToast('请先登录','error'); return; }
  var msg = await confirmDialog({
    icon: '✨',
    title: '申请加入',
    message: '写一段申请理由，让负责人更了解你。',
    prompt: true,
    placeholder: '如：自我介绍、想加入的原因、可参与的时间',
    confirmText: '提交申请'
  });
  if (msg === null) return;
  if (!msg.trim()) { showToast('请填写申请理由','error'); return; }
  try {
    var d = await apiFetch('/clubs/' + id + '/join', { method: 'POST', body: { message: msg } });
    showToast(d.message);
    setTimeout(function(){ window.location.reload(); }, 500);
  } catch(e) { showToast(e.message, 'error'); }
}

async function quickJoinClub(id) {
  if (!getToken()) { showToast('请先登录','error'); return; }
  var msg = await confirmDialog({
    icon: '✨',
    title: '申请加入',
...
async function leaveClub(id) {
  ...
  var ok = await confirmDialog({
    icon: '👋',
    title: '退出社团',
    message: '确认退出吗？退出后如需重新加入，需要再次提交申请。',
    confirmText: '确认退出'
  });
  if (!ok) return;
  try {
    var d = await apiFetch('/clubs/' + id + '/leave', { method: 'POST' });
    showToast(d.message || '已退出社团');
    setTimeout(function(){ window.location.reload(); }, 500);
  } catch(e) { showToast(e.message, 'error'); }
}
...
function claimClub(id) {
  ...
  if (!m) {
    ...
    m.innerHTML = '<div style="background:#fff;border-radius:16px;padding:1.2rem;max-width:480px;width:90%;" onclick="event.stopPropagation()">' +
      '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:.8rem;"><h3 style="font-weight:700;font-size:1rem;">🏷️ 认领组织</h3><button onclick="document.getElementById(\'claimModal\').style.display=\'none\'" style="border:none;background:transparent;font-size:1.2rem;cursor:pointer;color:#999;">✕</button></div>' +
      '<p style="font-size:.82rem;color:var(--text-tertiary);margin-bottom:.6rem;">请提供你与该组织关系的证明材料，以便管理员核实</p>' +
...
async function submitClaim() {
  ...
  if (!msg) { document.getElementById('claimError').textContent = '请填写你与该组织的关系说明'; return; }
